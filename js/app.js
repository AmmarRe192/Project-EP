// ── PAGE LOGIC (index.html only) ─────────────────────────
// Contact form, hours modal, admin/gallery (Supabase), floating
// index tabs, and the live open/closed status readout.
...
// ── CLICK TRACKING (porosit.mk redirect button) ──────────
// Fires a fire-and-forget ping to our own backend (/api/track-click),
// which uses a secret Supabase service-role key server-side to
// increment a counter. Never blocks the link from opening.
function trackPorositClick() {
  try {
    const payload = JSON.stringify({ key: 'porosit_redirect' });
  ...
  } catch (e) { /* never let tracking break the button */ }
}
...
// Apply saved language (script.js already restored theme + scroll animations)
setLang(localStorage.getItem('elixir-lang') || 'sq');
...
// ── LIVE OPEN / CLOSED STATUS (rail) ──────────────────────
// Mon–Sat 08:00–24:00, Sun 14:00–24:00 — mirrors the hours modal above.
function computeOpenStatus() {
  const now = new Date();
  const day = now.getDay(); // 0 = Sunday
  const minutes = now.getHours() * 60 + now.getMinutes();
  const openFrom = (day === 0) ? 14 * 60 : 8 * 60;
  const closeAt  = 24 * 60;
  const isOpen = minutes >= openFrom && minutes < closeAt;
  return { isOpen, openFrom, closeAt };
}
...
function renderOpenStatus() {
  ...
  if (!el) return;
  const dict = (typeof T !== 'undefined' && T[currentLang]) ? T[currentLang] : null;
  const { isOpen, openFrom, closeAt } = computeOpenStatus();
  el.classList.toggle('is-closed', !isOpen);
  const stateWord = dict ? (isOpen ? dict.status_open : dict.status_closed) : (isOpen ? 'Open now' : 'Closed');
  const timeWord  = dict ? (isOpen ? dict.status_closes : dict.status_opens) : (isOpen ? 'Closes at' : 'Opens at');
  const timeVal   = isOpen ? formatHM(closeAt) : formatHM(openFrom);
  el.innerHTML = `<span><span class="dot"></span><strong>${stateWord}</strong></span><span>${timeWord} ${timeVal}</span>`;
}

renderOpenStatus();
setInterval(renderOpenStatus, 60000);
// re-render the status line whenever the language changes
